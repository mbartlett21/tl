...
describe("warning flags", function()
   ...
   describe("in cli arguments", function()
      describe("--wdisable", function()
         ...
         it("throws error when given invalid warnings", function()
            local pd = io.popen(util.tl_cmd("check", "/dev/null", "--wdisable", "foo") .. "2>&1", "r")
            local output = pd:read("*a")
            util.assert_popen_close(1, pd:close())
            assert.match("Error: argument for option", output)
         end)
      end)
      describe("--werror", function()
         ...
         it("throws error when given invalid warnings", function()
            local pd = io.popen(util.tl_cmd("check", "/dev/null", "--werror", "foo") .. "2>&1", "r")
            local output = pd:read("*a")
            util.assert_popen_close(1, pd:close())
            assert.match("Error: argument for option", output)
         end)
